    ...
    for root_src, dirs, files in os.walk("lib/Crypto"):
        ...
        for file_name in files:
            ...
            with open(full_file_name_dst, "rt", **extra_param) as fd:
                content = (
                    fd.read().replace("Crypto.", "Cryptodome.").replace("Crypto ", "Cryptodome ").
                    replace("'Crypto'", "'Cryptodome'").replace('"Crypto"', '"Cryptodome"')
                )
...
with open(Path("lib") / package_root / "__init__.py") as init_root:
...
setup(
    ...
    cmdclass={
        'build_ext': PCTBuildExt, 'build_py': PCTBuildPy, 'test': TestCommand,
    },
