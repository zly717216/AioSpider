...
class RetryMiddleware(Middleware):
    ...
    def process_response(self, response):
        ...
        # 判断是否要重试
        if not getattr(self.settings, 'RETRY_ENABLE'):
            return None

        if response.status not in getattr(self.settings, 'RETRY_STATUS', []):
            return None

        if getattr(self.settings, 'MAX_RETRY_TIMES') < self.failure_pool.get_failure_times(response.request):
            self.failure_pool.remove_request(response.request)
            return False
...
class ProxyMiddleware(Middleware):
    ...
    def process_request(self, request):
        ...
        if request.proxy:
            proxy = request.proxy
        elif getattr(self.settings, "REQUEST_PROXY"):
            proxy = getattr(self.settings, "REQUEST_PROXY")
        else:
            proxy = None
        ...
        if isinstance(proxy, str):
            if 'http' in proxy:
                ...
                request.proxy = 'http://' + proxy.split('/')[-1]
        else:
            request.proxy = None
